...
@section('title', 'Create Business Permit Type')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0">Create Business Permit Type</h1>
        <a href="{{ route('admin.business-permit-types.index') }}" class="btn btn-secondary">
            <i class="fas fa-arrow-left me-2"></i>Back to Permit Types
        </a>
    </div>

    <form action="{{ route('admin.business-permit-types.store') }}" method="POST">
        @csrf
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="category" class="form-select @error('category') is-invalid @enderror" required>
                                    ...
                                    <option value="micro" {{ old('category') == 'micro' ? 'selected' : '' }}>Micro Enterprise</option>
                                    <option value="small" {{ old('category') == 'small' ? 'selected' : '' }}>Small Enterprise</option>
                                    <option value="medium" {{ old('category') == 'medium' ? 'selected' : '' }}>Medium Enterprise</option>
                                    <option value="large" {{ old('category') == 'large' ? 'selected' : '' }}>Large Enterprise</option>
                                    <option value="home_based" {{ old('category') == 'home_based' ? 'selected' : '' }}>Home Based</option>
                                    <option value="street_vendor" {{ old('category') == 'street_vendor' ? 'selected' : '' }}>Street Vendor</option>
                                </select>
                                @error('category')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" name="base_fee" class="form-control @error('base_fee') is-invalid @enderror" 
                                       value="{{ old('base_fee', 0) }}" min="0" max="999999.99" required>
                                @error('base_fee')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="processing_days" class="form-control @error('processing_days') is-invalid @enderror" 
                                       value="{{ old('processing_days', 7) }}" min="1" max="365" required>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="validity_months" class="form-control @error('validity_months') is-invalid @enderror" 
                                       value="{{ old('validity_months', 12) }}" min="1" max="60" required>
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <div id="additional-fees-container">
                            @if(old('additional_fees'))
                                @foreach(old('additional_fees') as $index => $fee)
                                    <div class="row mb-2 fee-row">
                                        <div class="col-md-7">
                                            <input type="text" class="form-control" 
                                                   name="additional_fees[{{ $index }}][name]" 
                                                   value="{{ $fee['name'] ?? '' }}"
                                                   placeholder="Fee name (e.g., Sanitary Fee)">
                                        </div>
                                        <div class="col-md-4">
                                ...
                                @foreach(old('requirements') as $requirement)
                                    <div class="input-group mb-2">
                                        <input type="text" class="form-control" name="requirements[]" 
                                               value="{{ $requirement }}" placeholder="e.g., Barangay Clearance">
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <div id="template-fields-container">
                            @if(old('template_fields'))
                                @foreach(old('template_fields') as $field)
                                    <div class="input-group mb-2">
                                        <input type="text" class="form-control" name="template_fields[]" 
                                               value="{{ $field }}" placeholder="e.g., Business Address">
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <textarea name="template_content" rows="10" class="form-control @error('template_content') is-invalid @enderror" 
                                  placeholder="Enter template content...">{{ old('template_content') }}</textarea>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-check mb-3">
                            <input type="checkbox" name="requires_fire_safety" value="1" 
                                   ...
                                   {{ old('requires_fire_safety') ? 'checked' : '' }}>
                        ...
                        <div class="form-check mb-3">
                            <input type="checkbox" name="requires_health_permit" value="1" 
                                   ...
                                   {{ old('requires_health_permit') ? 'checked' : '' }}>
                        ...
                        <div class="form-check mb-0">
                            <input type="checkbox" name="requires_environmental_clearance" value="1" 
                                   ...
                                   {{ old('requires_environmental_clearance') ? 'checked' : '' }}>
                    ...
                    <div class="card-body">
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-2"></i>Create Permit Type
                            </button>
                            <a href="{{ route('admin.business-permit-types.index') }}" class="btn btn-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Additional Fees
    let feeIndex = {{ old('additional_fees') ? count(old('additional_fees')) : 0 }};
    document.getElementById('add-fee').addEventListener('click', function() {
        ...
        row.innerHTML = `
            <div class="col-md-7">
                <input type="text" class="form-control" 
                       name="additional_fees[${feeIndex}][name]" 
                       placeholder="Fee name">
            </div>
            <div class="col-md-4">
                <input type="number" step="0.01" class="form-control" 
                       name="additional_fees[${feeIndex}][amount]" 
                       placeholder="Amount">
            </div>
            <div class="col-md-1">
                <button type="button" class="btn btn-outline-danger w-100 remove-fee">
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.remove-template-field')) {
            e.target.closest('.input-group').remove();
        }
    });
